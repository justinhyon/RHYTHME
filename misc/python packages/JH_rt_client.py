...
class TeamFlow:
    ...
    def receive_data(self):
        while True:
            ...
            if currentsamp == self.prevsamp + self.blocksize:
                ...
                # p1 = Process(target=self.psd_par(raw))
                #
                # p2 = Process(target=self.aep_par(raw))
                #
                # p3 = Process(target=self.plv_par(raw))
                # print(raw.info)
                # p1a = pool.apply_async(self.psd_par, raw)
                # p2a = pool.apply_async(self.aep_par, raw)
                # p3a = pool.apply_async(self.plv_par, raw)

                # p2a = pool.apply_async(self.psd, (raw, 0))
                # p3a = pool.apply_async(self.psd, (raw, 1))
                #
                # # p1a.get()
                # p2a.get()
                # p3a.get()
    ...
    def generate_sim_events(self, raw):
        ...
        E = mne.find_events(raw, stim_channel='Status')
    ...
    def final(self):
        print(len(self.AEPs1))
        AEPs1 = self.moving_average(self.AEPs1, p = True, norm=True)
        AEPs2 = self.moving_average(self.AEPs2, norm=True)
        # for i, n in enumerate(AEPs1):
        #     AEPs1[i] = AEPs1[i] * 1000000
        # for i, n in enumerate(AEPs2):
        #     AEPs2[i] = AEPs2[i] * 1000000
        ...
        # ls1 = [i for i, e in enumerate(AEPs1) if e != 0]
        # ls2 = [i for i, e in enumerate(AEPs2) if e != 0]
        # idx1 = ls1[-1]
        # idx2 = ls2[-1]
        ...
        intra1finals = self.moving_average(self.intra1finals, p = True, norm=True)
        intra2finals = self.moving_average(self.intra2finals, p = True, norm=True)
        interfinals = self.moving_average(self.interfinals, p = True, norm=True)
        ...
        x = np.arange(1, len(intra1finals) + 1)
        ...
        self.ax2[3, 0].bar(x, intra1finals[-1], width=0.4, color='red')  # index for plv intra 1
        self.ax2[3, 1].bar(x, interfinals[-1], width=0.4, color='blue')  # index for plv inter
        self.ax2[3, 2].bar(x, intra2finals[-1], width=0.4, color='green')  # index for plv intra 2
